...
Alphabet v2.3.6 uses a stack-based bytecode interpreter (no JIT). Performance characteristics:
...
Integer-typed operations (v2.3.6 real int64_t support) significantly improved performance over the previous double-only implementation.
...
- Real integer (int64_t) support since v2.3.6 avoids float overhead
