...
class FFIBridge {
  public:
    ...
    void unload_all();
    FFIArg call(const std::string& lib_path, const std::string& func_name, const std::vector<FFIArg>& args);

    template <typename Func> bool register_function(const std::string& name, Func func);
