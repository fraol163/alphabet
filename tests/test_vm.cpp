...
TEST_CASE("Division by zero throws RuntimeError", "[vm][negative][errors]")
{
    REQUIRE_THROWS_AS(
        test::run("#alphabet<test>\nz.o(10 / 0)"),
        RuntimeError);
}

TEST_CASE("Stack overflow throws RuntimeError", "[vm][negative][errors]")
{
    // Recursive function with no base case, exceeding MAX_CALL_DEPTH=1000
    REQUIRE_THROWS_AS(
        test::run(R"(#alphabet<test>
m 5 inf() {
  r inf()
}
inf())"),
